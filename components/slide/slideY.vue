<template>
<div class="slideY" @mouseenter="close" @mouseleave="run" :style="{ height }">
...
export default {
    ...
    methods:{
        ...
        enter(i){
            this.index = i;
            animate(this.$refs.box,[{ transform:{ translateY:-this.meanValue*this.index+'%' } }]);
        },
        up(){
            if(this.bool) return;
            this.bool = true;
            ...
            if(this.index==0){
                ...
                animate(this.$refs.box,[{ transform:{ translateY:-this.meanValue*i+'%' } }]).then(()=>{
            ...
            animate(this.$refs.box,[{ transform:{ translateY:-this.meanValue*this.index+'%' } }]).then(()=>{
                this.bool = false;
            });
        },
        next(){
